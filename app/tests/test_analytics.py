...
class AnalyticsClicksTestCase(AnalyticsSetupMixin, BaseTestCase):
    ...
    @unittest_run_loop
    async def test_order_year_clicks_returns_default_year_data(self):
        url = self.app.get_url(EndpointsMapper.AD_ORDER_YEAR_CLICKS, parts={'order_id': self.order_id})
        response = await self.client.get(url, headers=self.provider_headers)
    ...
    async def test_order_year_clicks_returns_paramed_year(self):
        url = self.app.get_url(EndpointsMapper.AD_ORDER_YEAR_CLICKS,
                               parts={'order_id': self.order_id},
                               query={'year': 2014})
        response = await self.client.get(url, headers=self.provider_headers)
    ...
    async def test_order_month_clicks_returns_default_month_and_year_data(self):
        url = self.app.get_url(EndpointsMapper.AD_ORDER_MONTH_CLICKS, parts={'order_id': self.order_id})
        response = await self.client.get(url, headers=self.provider_headers)
    ...
    async def test_order_month_clicks_returns_param_year_and_month(self):
        url = self.app.get_url(EndpointsMapper.AD_ORDER_MONTH_CLICKS,
                               parts={'order_id': self.order_id},
    ...
    async def test_order_day_clicks_returns_default_day_month_and_year_data(self):
        url = self.app.get_url(EndpointsMapper.AD_ORDER_DAY_CLICKS, parts={'order_id': self.order_id})
        response = await self.client.get(url, headers=self.provider_headers)
    ...
    async def test_order_day_clicks_returns_param_day_month_and_year_data(self):
        url = self.app.get_url(EndpointsMapper.AD_ORDER_DAY_CLICKS,
                               parts={'order_id': self.order_id},
...
class AnalyticsViewsTestCase(AnalyticsSetupMixin, BaseTestCase):
    ...
    @unittest_run_loop
    async def test_order_year_views_returns_default_year_data(self):
        url = self.app.get_url(EndpointsMapper.AD_ORDER_YEAR_VIEWS, parts={'order_id': self.order_id})
        response = await self.client.get(url, headers=self.provider_headers)
    ...
    async def test_order_year_views_returns_paramed_year(self):
        url = self.app.get_url(EndpointsMapper.AD_ORDER_YEAR_VIEWS,
                               parts={'order_id': self.order_id},
                               query={'year': 2014})
        response = await self.client.get(url, headers=self.provider_headers)
    ...
    async def test_order_month_views_returns_default_month_and_year_data(self):
        url = self.app.get_url(EndpointsMapper.AD_ORDER_MONTH_VIEWS, parts={'order_id': self.order_id})
        response = await self.client.get(url, headers=self.provider_headers)
    ...
    async def test_order_month_views_returns_param_year_and_month(self):
        url = self.app.get_url(EndpointsMapper.AD_ORDER_MONTH_VIEWS,
                               parts={'order_id': self.order_id},
                               query={'year': 2014, 'month': 4})
        response = await self.client.get(url, headers=self.provider_headers)
    ...
    async def test_order_day_views_returns_default_day_month_and_year_data(self):
        url = self.app.get_url(EndpointsMapper.AD_ORDER_DAY_VIEWS, parts={'order_id': self.order_id})
        response = await self.client.get(url, headers=self.provider_headers)
    ...
    async def test_order_day_views_returns_param_day_month_and_year_data(self):
        url = self.app.get_url(EndpointsMapper.AD_ORDER_DAY_VIEWS,
                               parts={'order_id': self.order_id},
                               query={'year': 2014, 'month': 5, 'day': 5})
        response = await self.client.get(url, headers=self.provider_headers)
